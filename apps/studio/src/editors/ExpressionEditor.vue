...
const text = computed(() => {
  ...
  if (typeof props.modelValue === 'object' && props.modelValue !== null && 'text' in props.modelValue) {
...
function updateText(value: string): void {
  const next = typeof props.modelValue === 'string' && props.schema.type !== 'object' ? value : { text: value };
...
function validationSpec(expression: string): PipelineSpec {
  const fields = fieldHints.value.map((field) => ({ name: field.value, type: { kind: field.kind } as const }));
  return {
    ...
    nodes: [
      ...
      { id: 'formula', operation: 'calculation.formula', config: { columns: [{ name: '结果', expression: { text: expression } }] } },
    ],
...
async function validateExpression(expression: string, sequence: number): Promise<void> {
  ...
  try {
    ...
    const expressionDiagnostics = result.diagnostics.filter((item) =>
      item.nodeId === 'formula' || item.code.startsWith('EXPRESSION_'));
    validationMessages.value = expressionDiagnostics.map(localizeDiagnostic);
    validationState.value = expressionDiagnostics.some((item) => item.severity === 'error') ? 'invalid' : 'valid';
    emit('diagnostics', expressionDiagnostics);
  } catch (cause: unknown) {
    if (sequence !== validationSequence) return;
    validationState.value = 'invalid';
    validationMessages.value = [cause instanceof Error ? cause.message : '表达式校验暂不可用。'];
  }
}
...
watch(text, (value) => {
  ...
  validationTimer = setTimeout(() => void validateExpression(value, sequence), VALIDATION_DELAY_MS);
}, { immediate: true });
...
<template>
  <div class="expression-editor">
    ...
    <div class="expression-editor__hints" aria-label="表达式提示">
      <span>字段</span>
      <button v-for="field in fieldHints" :key="field.value" type="button" :disabled="disabled" @click="insertHint(field.value, false)">{{ field.label }}</button>
      <span>函数</span>
      <button v-for="fn in builtInFunctions" :key="fn" type="button" :disabled="disabled" @click="insertHint(fn, true)">{{ fn }}</button>
    </div>
    <div class="expression-editor__status" :class="`expression-editor__status--${validationState}`" role="status" aria-live="polite">
      <span v-if="validationState === 'checking'">正在校验表达式…</span>
      <span v-else-if="validationState === 'valid'">表达式有效</span>
      <span v-else-if="validationState === 'invalid'">{{ validationMessages.join('；') }}</span>
