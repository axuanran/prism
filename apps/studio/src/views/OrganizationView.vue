...
const currentType = computed(() => resourceTypes.value.find((item) => item.kind === typeKindByTab[activeTab.value]));
...
const currentPresentation = computed<PresentationSpec | undefined>(() => {
  ...
  if (activeTab.value === 'units') {
    fields.parentId = { ...fields.parentId, widget: 'reference', editorOptions: { referenceKind: 'units' } };
  }
  if (activeTab.value === 'assignments') {
    fields.personId = { ...fields.personId, widget: 'reference', editorOptions: { referenceKind: 'people' } };
    fields.organizationUnitId = { ...fields.organizationUnitId, widget: 'reference', editorOptions: { referenceKind: 'units' } };
...
const referenceLoader: ReferenceLoader = async (request) => {
  if (request.kind === 'people') {
    return people.value.map((person) => ({ value: person.id, label: `${person.displayName}（${person.employeeNumber}）` }));
  }
  if (request.kind === 'units') {
    return units.value.map((unit) => ({ value: unit.id, label: `${unit.name}（${unit.code}）` }));
  }
  return [];
};
  ...
  try {
    if (activeTab.value === 'people') {
      ...
      await api.createUnit({
        ...
        ...(typeof value.through === 'string' && value.through ? { through: value.through } : {}),
      });
    } else {
      await api.createAssignment({
        personId: String(value.personId ?? ''),
        organizationUnitId: String(value.organizationUnitId ?? ''),
        kind: value.kind === 'secondary' ? 'secondary' : 'primary',
        from: String(value.from ?? ''),
        ...(typeof value.through === 'string' && value.through ? { through: value.through } : {}),
      });
    }
  ...
  <div>
    <div class="page-intro">
      <div class="page-intro__copy">
        ...
        <h2>组织与人员</h2>
        <p class="page-intro__description">维护人员、机构和生效期归属。表单随引擎配置契约自动生成。</p>
      </div>
      <button class="button button--primary" type="button" :disabled="loading" @click="openCreate">新建{{ tabLabel(activeTab) }}</button>
    </div>

    <EngineDataBoundary :loading="loading" :error="error" @retry="load">
      <div class="organization-layout">
        <section class="panel organization-directory" aria-labelledby="organization-tabs-title">
          <div class="organization-tabs" role="tablist" aria-label="组织数据分类">
            ...
            >
              {{ tabLabel(tab) }}
              <span>{{ tab === 'people' ? people.length : tab === 'units' ? units.length : assignments.length }}</span>
            </button>
          </div>
          ...
          <div class="directory-content">
            <div v-if="activeTab === 'people'" class="table-scroll">
              <table v-if="people.length > 0">
                <thead><tr><th scope="col">姓名</th><th scope="col">工号</th><th scope="col">职称</th><th scope="col">状态</th></tr></thead>
                <tbody>
                  <tr v-for="person in people" :key="person.id">
                    <th scope="row">{{ person.displayName }}</th>
                    <td>{{ person.employeeNumber }}</td>
                    <td>{{ person.title ?? '未填写' }}</td>
                    <td><span class="status-badge">{{ statusLabel(person.status) }}</span></td>
                  </tr>
                </tbody>
              </table>
              <div v-else class="business-empty"><strong>尚无人员</strong><p>新建第一位人员，开始维护组织关系。</p></div>
            </div>

            <div v-else-if="activeTab === 'units'" class="table-scroll">
              <table v-if="units.length > 0">
                <thead><tr><th scope="col">机构名称</th><th scope="col">机构编码</th><th scope="col">上级机构</th><th scope="col">生效日期</th></tr></thead>
                <tbody>
                  <tr v-for="unit in units" :key="unit.id">
                    <th scope="row">{{ unit.name }}</th>
                    <td>{{ unit.code }}</td>
                    <td>{{ unit.parentId ? unitById.get(unit.parentId)?.name ?? '上级机构已停用' : '顶级机构' }}</td>
                    <td>{{ unit.effectivePeriod.from }}</td>
                  </tr>
                </tbody>
              </table>
              <div v-else class="business-empty"><strong>尚无机构</strong><p>先建立机构，再为人员设置归属。</p></div>
            </div>

            <div v-else class="table-scroll">
              <table v-if="assignments.length > 0">
                <thead><tr><th scope="col">人员</th><th scope="col">所属机构</th><th scope="col">归属类型</th><th scope="col">生效期</th></tr></thead>
                <tbody>
                  <tr v-for="assignment in assignments" :key="assignment.id">
                    <th scope="row">{{ personById.get(assignment.personId)?.displayName ?? '人员已停用' }}</th>
                    <td>{{ unitById.get(assignment.organizationUnitId)?.name ?? '机构已停用' }}</td>
                    <td>{{ assignmentKindLabel(assignment.kind) }}</td>
                    <td>{{ assignment.effectivePeriod.from }} 起</td>
                  </tr>
                </tbody>
              </table>
              <div v-else class="business-empty"><strong>尚无人员归属</strong><p>选择人员和机构，建立带生效日期的关系。</p></div>
            </div>
          </div>
        </section>

        <aside v-if="creating && currentType" class="panel organization-form" aria-labelledby="organization-form-title">
          <div class="panel__header">
            <div><h3 id="organization-form-title">新建{{ tabLabel(activeTab) }}</h3><p>填写内容将直接保存到当前业务环境。</p></div>
            <button class="icon-button" type="button" aria-label="关闭表单" @click="creating = false">×</button>
          </div>
          <form @submit.prevent="submit">
            ...
            <div class="form-actions">
              <button class="button button--secondary" type="button" @click="creating = false">取消</button>
