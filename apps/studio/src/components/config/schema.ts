...
export function schemaType(schema: JsonSchema, root: JsonSchema): string {
  ...
  if (resolved.enum || resolved.const !== undefined)
    return typeof (resolved.enum?.[0] ?? resolved.const);
  const alternatives = schemaAlternatives(resolved);
  if (alternatives.length > 0) {
    return schemaType(
      alternatives.find((item) => item.const !== null) ?? alternatives[0] ?? {},
      root,
    );
  }
  return "string";
...
export function schemaSemantic(
  ...
  if (properties) {
    ...
    if (
      keys.length === 1 &&
      keys[0] === "text" &&
      schemaType(properties.text ?? {}, root) === "string"
    ) {
...
function defaultObject(schema: JsonSchema, root: JsonSchema): Record<string, unknown> {
  ...
  for (const [key, child] of Object.entries(schema.properties ?? {})) {
    ...
    if (required || child.default !== undefined || childDefault !== undefined)
...
function validateScalar(
  ...
  if (type === "string" && typeof value !== "string")
    diagnostics.push(diagnostic(path, "FORM_TYPE_STRING", "请输入文字。"));
  if ((type === "number" || type === "integer") && typeof value !== "number")
    diagnostics.push(diagnostic(path, "FORM_TYPE_NUMBER", "请输入数字。"));
  if (type === "integer" && typeof value === "number" && !Number.isInteger(value))
    diagnostics.push(diagnostic(path, "FORM_TYPE_INTEGER", "请输入整数。"));
  if (type === "boolean" && typeof value !== "boolean")
    diagnostics.push(diagnostic(path, "FORM_TYPE_BOOLEAN", "请选择开启或关闭。"));
  if (typeof value === "string") {
    if (schema.minLength !== undefined && value.length < schema.minLength)
      diagnostics.push(diagnostic(path, "FORM_MIN_LENGTH", "此项不能为空。"));
    if (schema.maxLength !== undefined && value.length > schema.maxLength)
      diagnostics.push(
        diagnostic(path, "FORM_MAX_LENGTH", `最多填写 ${schema.maxLength} 个字符。`),
      );
    if (schema.pattern) {
      try {
        if (!new RegExp(schema.pattern).test(value))
          diagnostics.push(diagnostic(path, "FORM_PATTERN", "填写格式不正确。"));
      } catch {
  ...
  if (typeof value === "number") {
    if (schema.minimum !== undefined && value < schema.minimum)
      diagnostics.push(diagnostic(path, "FORM_MINIMUM", `不能小于 ${schema.minimum}。`));
    if (schema.maximum !== undefined && value > schema.maximum)
      diagnostics.push(diagnostic(path, "FORM_MAXIMUM", `不能大于 ${schema.maximum}。`));
  }
  const choices = schemaEnum(schema);
  if (choices.length > 0 && !choices.some((choice) => Object.is(choice, value)))
    diagnostics.push(diagnostic(path, "FORM_ENUM", "请选择列表中的选项。"));
  return diagnostics;
}
...
export function validateValue(
  ...
  if (type === "object") {
    if (typeof value !== "object" || value === null || Array.isArray(value))
      return [diagnostic(path, "FORM_TYPE_OBJECT", "此部分配置格式不正确。")];
    ...
    for (const [key, child] of Object.entries(resolved.properties ?? {})) {
      diagnostics.push(
        ...validateValue(child, object[key], root, path ? `${path}.${key}` : key),
      );
    }
    return diagnostics;
  }
  if (type === "array") {
    if (!Array.isArray(value))
      return [diagnostic(path, "FORM_TYPE_ARRAY", "此项应为列表。")];
    const diagnostics: Diagnostic[] = [];
    if (resolved.minItems !== undefined && value.length < resolved.minItems)
      diagnostics.push(
        diagnostic(path, "FORM_MIN_ITEMS", `至少添加 ${resolved.minItems} 项。`),
      );
    if (resolved.maxItems !== undefined && value.length > resolved.maxItems)
      diagnostics.push(
        diagnostic(path, "FORM_MAX_ITEMS", `最多添加 ${resolved.maxItems} 项。`),
      );
    if (resolved.items)
      value.forEach((item, index) =>
        diagnostics.push(
          ...validateValue(resolved.items ?? {}, item, root, `${path}.${index}`),
        ),
      );
    return diagnostics;
  }
