...
const semantic = computed(
  () =>
    effectivePresentation.value.editor ??
...
const widget = computed(() => {
  ...
  if (
    schema["x-prism-reference"] ||
    schema.format === "resource-reference" ||
...
const readonly = computed(
  () => props.disabled || effectivePresentation.value.readonly === true,
);
const label = computed(
  () =>
    effectivePresentation.value.label ??
    resolvedSchema.value.title ??
    businessFieldLabel(props.fieldKey),
);
const help = computed(
  () => effectivePresentation.value.help ?? resolvedSchema.value.description,
);
const exactDiagnostics = computed(() =>
  props.diagnostics.filter(
    (item) => normalizeFieldPath(item.path ?? "") === normalizeFieldPath(props.path),
  ),
);
...
const arrayValue = computed<readonly unknown[]>(() =>
  Array.isArray(props.modelValue) ? props.modelValue : [],
);
const childFields = computed(() =>
  objectFields(resolvedSchema.value, props.rootSchema, props.presentation, props.path),
);
const referenceKind = computed(() => {
  ...
  if (resolvedSchema.value.$ref && !resolvedSchema.value.$ref.startsWith("#/"))
    return resolvedSchema.value.$ref;
...
function updateArray(index: number, value: unknown): void {
  emit(
    "update:modelValue",
    arrayValue.value.map((item, itemIndex) => (itemIndex === index ? value : item)),
  );
}
...
async function loadReferences(): Promise<void> {
  ...
  try {
    referenceOptions.value = await props.referenceLoader({
      path: props.path,
...
<template>
  <div
    v-if="!effectivePresentation.hidden"
    class="config-field"
    :class="[
      `config-field--${type}`,
      { 'config-field--error': exactDiagnostics.length > 0 },
    ]"
  >
        ...
        <li v-for="(item, index) in arrayValue" :key="index">
          <div class="config-array__item-heading">
            ...
            <div>
              ...
              <button
                type="button"
                :disabled="readonly || index === arrayValue.length - 1"
                aria-label="下移"
                @click="moveArrayItem(index, 1)"
              >
                ↓
              </button>
              <button type="button" :disabled="readonly" @click="removeArrayItem(index)">
                移除
              </button>
            </div>
          </div>
    ...
    <template v-else>
      ...
      <input
        ...
        "
        :inputmode="
          resolvedSchema['x-prism-decimal'] || resolvedSchema.format === 'decimal'
            ? 'decimal'
            : undefined
        "
        ...
        @input="updateInput"
      />
      <p v-if="referenceError" class="config-field__error" role="alert">
        {{ referenceError }}
      </p>
    </template>

    <ul
      v-if="exactDiagnostics.length > 0"
      :id="errorId"
      class="config-field__errors"
      role="alert"
    >
      <li v-for="item in exactDiagnostics" :key="`${item.code}:${item.message}`">
