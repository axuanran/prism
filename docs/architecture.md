...
Successful builds write content-addressed artifacts beneath `sha256/<prefix>/<hash>`, persist artifact metadata and logs, and publish an immutable `project.release` Resource. The Release fixes Source revision/fingerprint, package and lock hashes, Node/pnpm/builder versions, client/server/build-manifest artifacts, test result and exact built Code Material identities. Build success does not activate or execute the Release; App Runtime and Active Release CAS belong to Phase 4.
