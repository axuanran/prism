...
Project builds begin from an exact published Source revision. `project.build` stores a CAS-transitioned Build Request, forks an isolated Build Worker process, materializes the canonical tree, performs `pnpm install --frozen-lockfile`, TS7 `--noEmit`, Vitest, Vite client build and esbuild server/material builds, then verifies every declared export. Failed typechecks/tests/builds persist diagnostics and logs but create no Project Release.

`artifact.store@1.0.0` is the only Build/Runtime Artifact boundary. Build submits immutable file trees; Runtime uses `stat`, `read` and full SHA-256 `verify`. `artifact.store.local` implements the contract under `sha256/<prefix>/<hash>`. Build and Runtime depend on the contract—not on each other or on the local provider—so a Runtime host never installs Vite, Vitest or esbuild.

Successful builds publish an immutable `project.release` Resource. The Release fixes Source revision/fingerprint, package and lock hashes, Node/pnpm/builder versions, Runtime ABI, required client/server export names, Action identities, verified Artifact refs, test result and exact built Code Material identities. `buildReproducibility` describes the pinned toolchain; `runtimeReproducibility` remains `UNKNOWN` until each Action Run records observed semantics.
